...
# חשוב: העברת המודלים ל-GPU
models[0].to(device)
models[1].to(device)
models[0].eval()
models[1].eval()

# --- הרצת הערכה (Hard Voting Loop) ---
...
for i, batch_data in enumerate(test_iterator):
    with torch.no_grad():
        ...
        # חיזוי ישיר מהמודלים (בלי מחלקת Ensemble שלא הייתה בשימוש)
        y_pred_1_out = models[0](in_1)
        y_pred_2_out = models[1](in_2)
        y_pred_3_out = (y_pred_1_out + y_pred_2_out) / 2 # ממוצע
